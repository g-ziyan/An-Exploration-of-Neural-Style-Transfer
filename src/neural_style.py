...
def train(args):
    ...
    features_style = vgg(normalize_batch(style))[:-1]
...
def stylize(args):
    ...
    if args.original_colors:
        output = convert_to_original_colors(np.copy(content_image), output[0])

    save_image(args.output_image, output)
...
def main():
    ...
    eval_arg_parser.add_argument("--preserve_color", action='store_true',
